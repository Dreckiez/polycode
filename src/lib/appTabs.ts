...
import { basename, type GitFileDiffKind } from "./fs";
import {
  firstLeafId,
  focusedFileTab,
  isCommitTab,
...
import { releaseNotesTitle } from "./releaseNotes";
import {
  type HarnessId,
  type Session,
  sessionDisplayTitle,
...
export function isBlankWorkspaceTab(
  tab: WorkspaceTab,
  sessions: Session[],
): boolean {
...
export function selectedChangeKind(
  tab: WorkspaceTab,
): GitFileDiffKind | undefined {
  const file = focusedFileTab(tab);
